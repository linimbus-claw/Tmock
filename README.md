...
// 基本用法：参数匹配 + 返回值
EXPECT_CALL(m, login, "alice", Eq(1234))
    .Return(true);

// 通配符 + 自定义 matcher
EXPECT_CALL(m, query, An<std::string>())
    .with([](const std::string& s){ return s.find("SELECT") == 0; })
    .Return(42)
    .Times(3);

// 异步回调
EXPECT_CALL(m, asyncFetch, _, _)
    .Invoke([](const std::string&, std::function<void(const std::string&)> cb) {
        cb("mocked result");
    });

// 多次匹配
EXPECT_CALL(m, login, _, _)
    .Return(false)
    .AtLeast(2);
```
...
- **v1.1.0**: EXPECT_CALL 自动类型推导，无需手动指定 Ret 和 (ArgTypes...)
